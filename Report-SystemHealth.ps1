...
# This is dependent on the MS Chart Controls for MS .NET Framework 3.5.

#region Variables and Arguments
$users = "helpdesk@nacgroup.com" # List of users to email your report to (separate by comma)
...
$CurrentTime = Get-Date
    ...
    $ReceiveMailbox = "helpdesk@nacgroup.com"
...
foreach ($computer in $computers) {
    ...
    #region Available Updates Report
    $UpdatesAvailableReport = @()
    $UpdatesAvailable = Get-PendingUpdate -Computer $computer
    foreach ($update in $UpdatesAvailable) {
        ...
        $UpdatesAvailableReport += $row
    }

    $UpdatesAvailableReport = $UpdatesAvailableReport | ConvertTo-Html -Fragment
	#endregion
    ...
    <p>The following is a list of updates available through Windows updates.</p>
    <table class="normal">$UpdatesAvailableReport</table>
    <br></br>
...
Send-MailMessage -To $ReceiveMailbox -Subject "[Report] $ReportTitle" -BodyAsHTML -Body $HTMLmessage -Attachments $ListOfAttachments -UseSsl -Port 587 -SmtpServer smtp.office365.com -From $SourceMailbox -Credential $Creds
